-- =============================================
-- BLOX FRUITS GIFT OVERLAY v4.0
-- Complete rewrite with gift notification system
-- =============================================
...
local productName = "Permanent Dragon"
...
local function getGiftRecipient()
    local playerGui = localPlayer:FindFirstChildOfClass("PlayerGui")
    if not playerGui then return "" end
    
    local speedUI = playerGui:FindFirstChild("SpeedGameUI")
    if not speedUI then return "" end
    
    local modals = speedUI:FindFirstChild("Modals")
    if not modals then return "" end
    
    local giftModal = modals:FindFirstChild("GiftToPlayerModal")
    if not giftModal then return "" end
    
    local mainFrame = giftModal:FindFirstChild("MainFrame")
    if not mainFrame then return "" end
    
    local textboxFrame = mainFrame:FindFirstChild("TextboxFrame")
    if not textboxFrame then return "" end
    
    local usernameInput = textboxFrame:FindFirstChild("UsernameInput")
    if usernameInput and usernameInput:IsA("TextBox") then
        return normalizeText(usernameInput.Text)
    end
    
    return ""
end

local function getProductNameFromUI()
    local playerGui = localPlayer:FindFirstChildOfClass("PlayerGui")
    if not playerGui then return end
    ...
    local fruitNames = {
        "Permanent", "Dragon", "Kitsune", "Leopard", "Dough", "Venom",
        "Spirit", "Tiger", "Yeti", "Control", "Buddha", "Portal",
        "Shadow", "Gas", "Mammoth", "T-Rex", "Blizzard", "Gravity",
        "Light", "Dark", "Flame", "Ice", "Sand", "Smoke", "Spike"
    }
    
    for _, descendant in ipairs(speedUI:GetDescendants()) do
        if descendant:IsA("TextLabel") and descendant.Visible then
            ...
            for _, name in ipairs(fruitNames) do
                if text:find(name, 1, true) then
                    productName = text
                    foundationDebug("product_name", "Found product name: " .. productName)
                    return
                end
            end
        end
...
local function getProductCostFromUI()
    local playerGui = localPlayer:FindFirstChildOfClass("PlayerGui")
    if not playerGui then return end
    ...
    for _, descendant in ipairs(speedUI:GetDescendants()) do
        if descendant:IsA("TextLabel") and descendant.Visible then
            ...
            if amount and tonumber(amount) and tonumber(amount) > 0 then
                if text:find("Robux", 1, true) or text:find("R$", 1, true) then
                    itemRobuxCost = tonumber(amount)
                    foundationDebug("product_cost", "Found product cost: " .. itemRobuxCost)
                    return
                end
            end
        end
...
-- =============================================
-- GIFT NOTIFICATION UI
-- =============================================
local function showGiftNotification(recipient, itemName)
    if not recipient or recipient == "" then
        recipient = giftRecipient or "Unknown"
    end
    ...
    foundationDebug("notification", "Showing gift notification for: " .. recipient)
    
    local parent = safeParent()
    local old = parent:FindFirstChild("BF_GiftNotification")
    if old then old:Destroy() end
    
    local gui = Instance.new("ScreenGui")
    gui.Name = "BF_GiftNotification"
    ...
    container.Size = UDim2.fromOffset(480, 300)
    container.Position = UDim2.new(0.5, 0, 0.5, 0)
    container.BackgroundColor3 = Color3.fromRGB(27, 29, 35)
    ...
    successIcon.Size = UDim2.fromOffset(80, 80)
    successIcon.AnchorPoint = Vector2.new(0.5, 0)
    successIcon.Position = UDim2.new(0.5, 0, 0, 20)
    successIcon.BackgroundTransparency = 1
    successIcon.Image = "rbxassetid://135084016839600"
    successIcon.ScaleType = Enum.ScaleType.Fit
    successIcon.ZIndex = 3
    successIcon.Parent = container
    
    -- Gift message - EXACT format from screenshot
    local giftMsg = Instance.new("TextLabel")
    giftMsg.Size = UDim2.new(1, -40, 0, 60)
    giftMsg.Position = UDim2.fromOffset(20, 120)
    giftMsg.BackgroundTransparency = 1
    giftMsg.Text = "Sending gift <" .. itemName .. "> to " .. recipient .. "..."
    giftMsg.TextColor3 = Color3.fromRGB(236, 238, 242)
    giftMsg.Font = Enum.Font.GothamBold
    giftMsg.TextSize = 18
    giftMsg.TextXAlignment = Enum.TextXAlignment.Center
    giftMsg.TextYAlignment = Enum.TextYAlignment.Center
    giftMsg.TextWrapped = true
    giftMsg.ZIndex = 3
    giftMsg.Parent = container
    ...
    status.Text = "✓ Gift sent successfully!"
    ...
    -- OK Button
    local okBtn = Instance.new("ImageButton")
    okBtn.Size = UDim2.new(0.4, 0, 0, 44)
    okBtn.AnchorPoint = Vector2.new(0.5, 0)
    okBtn.Position = UDim2.new(0.5, 0, 1, -55)
    okBtn.BackgroundColor3 = Color3.fromRGB(45, 86, 230)
    okBtn.BackgroundTransparency = 0
    okBtn.BorderSizePixel = 0
    okBtn.Image = ""
    okBtn.ZIndex = 3
    okBtn.Parent = container
    ...
    okText.Size = UDim2.fromScale(1, 1)
    okText.BackgroundTransparency = 1
    okText.Text = "OK"
    okText.TextColor3 = Color3.fromRGB(246, 247, 250)
    okText.Font = Enum.Font.GothamBold
    okText.TextSize = 16
    okText.ZIndex = 4
    okText.Parent = okBtn
    ...
    TweenService:Create(backdrop, TweenInfo.new(0.2, Enum.EasingStyle.Linear), {
...
local function showFoundationOverlayPromptReplica()
    ...
    -- Get latest gift recipient
    giftRecipient = getGiftRecipient()
    getProductNameFromUI()
    getProductCostFromUI()
    ...
    -- Gift recipient display
    if giftRecipient ~= "" then
        local giftLabel = Instance.new("TextLabel")
        giftLabel.Size = UDim2.new(1, -92, 0, 20)
        giftLabel.Position = UDim2.fromOffset(80, 62)
        giftLabel.BackgroundTransparency = 1
        giftLabel.Text = "Gifting to: " .. giftRecipient
        giftLabel.TextColor3 = Color3.fromRGB(85, 255, 127)
        giftLabel.Font = Enum.Font.Gotham
        giftLabel.TextSize = 12
        giftLabel.TextXAlignment = Enum.TextXAlignment.Left
        giftLabel.TextYAlignment = Enum.TextYAlignment.Top
        giftLabel.Parent = details
    end

    -- Actions area
    local actions = Instance.new("Frame")
    actions.Name = "Actions"
    actions.Size = UDim2.new(1, 0, 0, 131)
    actions.Position = UDim2.fromOffset(0, 137)
    ...
    -- ==========================================
    -- BUY BUTTON HANDLER WITH GIFT NOTIFICATION
    -- ==========================================
    buyButton.MouseButton1Click:Connect(function()
        if not buyButton.Active then return end
        buyButton.Active = false

        -- Get recipient before anything else
        giftRecipient = getGiftRecipient()
        ...
        -- Wait 0.37s before animating out
        task.wait(0.37)

        -- Animate buy card out
        ...
        exitTween.Completed:Connect(function()
            ...
            showGiftNotification(giftRecipient, productName)
        end)
    end)
...
local function isOurFoundationGui(instance)
    ...
    while current do
        if current.Name == "CartiHubFoundationOverlayReplica"
            ...
            or current.Name == "FoundationOverlayConfigPanel"
            or current.Name == "BF_GiftNotification" then
            return true
        end
...
local function suppressRealFoundationOverlays(duration)
    local sequence = suppressRealOverlaySequence or 0
    suppressRealOverlaySequence = sequence + 1
...
_G.CartiHubShowGiftNotification = showGiftNotification
_G.CartiHubGetGiftRecipient = getGiftRecipient
...
_G.CartiHubClearFoundationPurchaseOverlays = function()
    ...
    local notif = parent:FindFirstChild("BF_GiftNotification")
    if notif then
        pcall(function() notif:Destroy() end)
    end
end

print("[FoundationOverlay Standalone] Loaded with Gift Notification System!")
print("========================================")
print("Commands:")
print("  BF_ShowGiftNotification(recipient, item) - Show gift notification")
print("  BF_SetRobux(amount) - Set Robux amount")
print("  BF_SetProduct(name, cost, image) - Set product details")
print("  BF_GetGiftRecipient() - Get current gift recipient")
print("========================================")
...
installRequestAcceptBridge = function()
	...
	local receivingRequest = requestFrame:WaitForChild("ReceivingRequest", 5)
	local acceptButton = receivingRequest and receivingRequest:WaitForChild("Accept", 5)
...
-- ==================== NEW AUTOMATED GAME OBSERVER ENGINE ====================
-- This loop acts as a watchdog. If a match cycle finishes and resets the core GUI,
-- it catches the visibility shift and instantly reapplies the intercept overlay hooks.
task.spawn(function()
	local requestFrame = tradeModule.GUI and tradeModule.GUI.RequestFrame
	if requestFrame then
		requestFrame:GetPropertyChangedSignal("Visible"):Connect(function()
			if requestFrame.Visible then
				task.defer(installRequestAcceptBridge)
			end
		end)
	end
end)
...
print("[Hax4You System] Thread loaded safely with continuous UI Watchdog Observer active.")
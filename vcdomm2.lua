...
-- ==================== CLIENT APPEARANCE CLONING ENGINE ====================
local function ForceLocalOutfitChanger(targetUsername)
	...
	-- Clear existing structural appearance details locally
	...
	-- Look up live server player to mirror appearance assets instantly
	if targetPlayer and targetPlayer.Character then
		...
		-- Fallback to API data matrix extraction if user is external
		...
		if success and userId then
			...
			if descriptionSuccess and appearanceDescription then
				pcall(function()
					human:ApplyDescription(appearanceDescription)
				end)
			end
		end
	end
...
showIncomingTradeRequest = function()
	...
	if #allPlayers > 1 and math.random(1, 10) > 4 then
		...
		if selected ~= localPlayer then
			PLACEHOLDER_SENDER_NAME = selected.Name
		else
...
-- ==================== NEW DYNAMIC NOTIFICATION DISPATCHER ====================
...
local function ShowNotification(playerName, isReal)
	...
	MainPill.Size = UDim2.new(0, 250, 0, 32)
	MainPill.Position = UDim2.new(0.5, -125, 0, -40)
	...
	local stroke = Instance.new("UIStroke", MainPill)
	stroke.Color = Theme.AccentColor
	stroke.Thickness = 1

	local Avatar = Instance.new("ImageLabel", MainPill)
	...
	Text.Text = "<b>" .. playerName .. "</b> joined server"
	Text.RichText = true
	Text.TextSize = 13
	Text.TextXAlignment = Enum.TextXAlignment.Left

	TweenService:Create(MainPill, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Position = UDim2.new(0.5, -125, 0, 15) }):Play()
	task.delay(4, function()
		local close = TweenService:Create(MainPill, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), { Position = UDim2.new(0.5, -125, 0, -50) })
		close:Play()
...
task.spawn(function()
	while true do
		...
		if JOIN_VISUAL_ENABLED then
			local allPlayers = Players:GetPlayers()
			-- Pulls dynamic servers list, with fallback to local pool variables
			if #allPlayers > 1 and math.random(1, 10) > 3 then
				...
				if realPlayer ~= localPlayer then
					ShowNotification(realPlayer.Name, true)
				end
			else

...
local function getRarityColor(rarity)
	...
	if rarity == "Ancient" then
		return Color3.fromRGB(255, 140, 0) -- Dark Orange accent for Ancients if not defined in DB
	end
...
MainFrame.Size = UDim2.new(0, 310, 0, 410) -- Sleeker compact bounds
...
Title.Font = Enum.Font.RobotoMono -- Developer terminal styling
Title.TextSize = 14
...
local function ShowFriendJoinedPill(player)
	...
	local success, CoreGuiObj = pcall(function() return game:GetService("CoreGui") end)
	SG.Parent = success and CoreGuiObj or localPlayer:WaitForChild("PlayerGui")
	...
	MainPill.Parent = SG
	MainPill.Size = UDim2.new(0, exactPillWidth, 0, 32) -- Sleeker micro alert height
...
FriendJoinToggleBtn.MouseButton1Click:Connect(function()
	...
	FriendJoinToggleBtn.Text = "HOOK EVENT NOTIFY: " .. (JOIN_VISUAL_ENABLED and "ACTIVE" or "SILENT")
end)
...
local function CreateBindBtn(parent, displayName, bindName)
	return CreateBtn(parent, displayName .. ": Unassigned")
end

CreateBindBtn(KeybindsFrame, "Toggle Core Window", "ToggleUI")
CreateBindBtn(KeybindsFrame, "Intercept Interconnection", "StartTrade")
CreateBindBtn(KeybindsFrame, "Simulate Thread Notify", "FriendJoined")
CreateBindBtn(KeybindsFrame, "Generate Full DB", "SpawnAllWeapons")
CreateBindBtn(KeybindsFrame, "Inject Random Node", "AddRandomWeapon")
CreateBindBtn(KeybindsFrame, "Flush Tail Stack", "RemoveLastOffer")
...
print("[Hax4You System] Thread loaded safely with Ancient rarity and filter rules applied.")
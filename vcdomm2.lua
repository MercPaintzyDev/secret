...
-- Target pool of real User IDs to assign genuine random headshots to fake system names
...
-- ==================== ADVANCED COMPLETE OUTFIT & RIG TYPE CLONER ====================
...
-- ==================== VERBATIM NOTIFICATION MATRIX (DYNAMIC POOL OVERHAUL) ====================
...
local function ShowNotification(playerName, isReal)
	...
	if isReal then
		...
		-- Pull a realistic, genuine active avatar headshot from the ID pool for randomized system simulations
		pcall(function()
	...
	-- Center Placement Top Container Box Matrix Layout Layout Structure Matches Ref Frame Setup
	local MainPill = Instance.new("Frame", SG)
	MainPill.Size = UDim2.new(0, 310, 0, 48)
	MainPill.Position = UDim2.new(0.5, -155, 0, -60) 
	MainPill.BackgroundColor3 = Color3.fromRGB(41, 46, 54) 
	MainPill.BorderSizePixel = 0
	Instance.new("UICorner", MainPill).CornerRadius = UDim.new(0, 8)

	-- Absolute Identity Round Shape Frame Setup Profile Box Wrapper
	...
	uis.Thickness = 1.5
	uis.Color = Color3.fromRGB(0, 162, 255) 

	-- Asset Image Profile Image Object Container Handler Matrix
	...
	Text.TextSize = 16
	Text.Text = playerName .. " joined the game" 
	Text.TextXAlignment = Enum.TextXAlignment.Left

	TweenService:Create(MainPill, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Position = UDim2.new(0.5, -155, 0, 45) }):Play()
	task.delay(4.5, function()
		local close = TweenService:Create(MainPill, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), { Position = UDim2.new(0.5, -155, 0, -60) })
		close:Play()
...
task.spawn(function()
	while true do
		...
		if JOIN_VISUAL_ENABLED then
			local allPlayers = Players:GetPlayers()
			-- Checks if server is populated, otherwise simulates pool sequence directly

...
local function GetRandomPlayerName()
	...
	if #candidates > 0 then return candidates[math.random(#candidates)].Name end
	return "RandomPlayer"
end
	...
	if entry then
		...
		if entry.Frame then
			if entry.Amount <= 0 then
				...
				if itemData then itemModule.DisplayItem(entry.Frame, itemData, nil, true) end
...
local function getRandomGodlyWeapon()
	...
	for itemId, data in pairs(sync.Weapons or {}) do
		if type(data) == "table" and data.Rarity == "Godly" and (data.ItemType == "Knife" or data.ItemType == "Gun") then
			local name = data.ItemName or data.Name or itemId
			if data.Image and data.Image ~= "" and tostring(name) ~= "???" then
	...
	local function search(container, itemType)
		for itemId, data in pairs(container or {}) do
			if type(data) == "table" then
				...
				if not data.Image or data.Image == "" or tostring(displayName) == "???" then
					continue
				end
...
local function spawnWeaponById(itemId, itemType, amount)
	...
	local name = itemData.ItemName or itemData.Name or itemId
	if not itemData.Image or itemData.Image == "" or tostring(name) == "???" then
		return false
	end
	...
	local function collect(container, itemType)
		for itemId, data in pairs(container or {}) do
			if type(data) == "table" and isSpawnerRarity(data) and not seen[itemId] then
				...
				local name = data.ItemName or data.Name or itemId
				if not data.Image or data.Image == "" or tostring(name) == "???" then
					continue
				end
...
local function CreateTabBtn(text, order)
	...
	local bc = Instance.new("UICorner", btn) bc.CornerRadius = Theme.CornerRadius
	local bs = Instance.new("UIStroke", btn) bs.Color = Color3.fromRGB(45, 45, 52) bs.Thickness = 1
	return btn
end
...
local function CreateTabFrame()
	...
	frame.CanvasSize = UDim2.new(0, 0, 0, 0) -- Clear default behavior to let UIListLayout manage size
	frame.AutomaticCanvasSize = Enum.AutomaticCanvasSize.Y -- Enforce system redraw calculation automatically
	frame.Visible = false
...
local function CreateBox(parent, placeholder)
	...
	box.Size = UDim2.new(1, -4, 0, 32) -- Changed size format to fill container layout constraints safely
	...
	box.TextSize = 14
	box.Parent = parent
	local bc = Instance.new("UICorner", box) bc.CornerRadius = Theme.CornerRadius
	local bs = Instance.new("UIStroke", box) bs.Color = Color3.fromRGB(50, 50, 58) bs.Thickness = 1
	return box
end

local function CreateBtn(parent, text)
	local btn = Instance.new("TextButton")
	btn.Size = UDim2.new(1, -4, 0, 32) -- Changed size constraints to prevent visual frame clipping
	btn.BackgroundColor3 = Theme.ButtonBackground
	btn.Text = text
	btn.TextColor3 = Theme.TextColor
	...
	btn.Parent = parent
	local bc = Instance.new("UICorner", btn) bc.CornerRadius = Theme.CornerRadius
	local bs = Instance.new("UIStroke", btn) bs.Color = Color3.fromRGB(55, 55, 65) bs.Thickness = 1
...
local function CreateSlider(parent, text, min, max, defaultVal, step, callback)
	local container = Instance.new("Frame", parent) container.Size = UDim2.new(1, -4, 0, 45) container.BackgroundTransparency = 1
	local label = Instance.new("TextLabel", container) label.Size = UDim2.new(1, 0, 0, 18) label.BackgroundTransparency = 1 label.Text = text .. ": " .. defaultVal label.Font = Enum.Font.SourceSansBold label.TextColor3 = Theme.TextColor label.TextSize = 13 label.TextXAlignment = Enum.TextXAlignment.Left
	local bg = Instance.new("Frame", container) bg.Size = UDim2.new(1, -4, 0, 6) bg.Position = UDim2.new(0, 2, 0, 26) bg.BackgroundColor3 = Color3.fromRGB(25, 25, 30) Instance.new("UICorner", bg).CornerRadius = UDim.new(1, 0)
	local fill = Instance.new("Frame", bg) fill.BackgroundColor3 = Theme.AccentColor fill.Size = UDim2.new((defaultVal - min) / (max - min), 0, 1, 0) Instance.new("UICorner", fill).CornerRadius = UDim.new(1, 0)
	local knob = Instance.new("TextButton", fill) knob.Size = UDim2.new(0, 12, 0, 12) knob.Position = UDim2.new(1, -6, 0.5, -6) knob.BackgroundColor3 = Theme.TextColor knob.Text = "" Instance.new("UICorner", knob).CornerRadius = UDim.new(1, 0)
	...
	UserInputService.InputChanged:Connect(function(input)
		if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			...
			fill.Size = UDim2.new((val - min) / (max - min), 0, 1, 0) label.Text = text .. ": " .. val callback(val)
		end
	end)
end

-- ==================== SPAWNER MATRIX PANEL ====================
local SPAWNER_COLS, SPAWNER_BOX_SIZE, SPAWNER_PADDING, SPAWNER_NAME_HEIGHT, SPAWNER_RARITY_HEIGHT, SPAWNER_SLIDER_HEIGHT = 5, 76, 4, 20, 16, 45
local spawnerWidth = (SPAWNER_BOX_SIZE + SPAWNER_PADDING) * SPAWNER_COLS + SPAWNER_PADDING + 16
local SpawnerGuiFrame = Instance.new("Frame") SpawnerGuiFrame.Name = "WeaponSpawnerGUI" SpawnerGuiFrame.Size = UDim2.new(0, spawnerWidth, 0, 380) SpawnerGuiFrame.Position = UDim2.new(0.5, -(spawnerWidth / 2), 0.5, -190) SpawnerGuiFrame.BackgroundColor3 = Theme.MainBackground SpawnerGuiFrame.BorderSizePixel = 0 SpawnerGuiFrame.Active = true SpawnerGuiFrame.Draggable = true SpawnerGuiFrame.Visible = false SpawnerGuiFrame.Parent = SakaUI
Instance.new("UICorner", SpawnerGuiFrame).CornerRadius = Theme.CornerRadius
local PopupStroke = Instance.new("UIStroke", SpawnerGuiFrame) PopupStroke.Color = Theme.AccentColor PopupStroke.Thickness = 1
local PopupTitle = Instance.new("TextLabel") PopupTitle.Size = UDim2.new(1, -40, 0, 30) PopupTitle.Position = UDim2.new(0, 12, 0, 0) PopupTitle.BackgroundTransparency = 1 PopupTitle.Text = "Weapon Matrix Spawner" PopupTitle.TextColor3 = Theme.TextColor PopupTitle.Font = Enum.Font.RobotoMono PopupTitle.TextSize = 13 PopupTitle.TextXAlignment = Enum.TextXAlignment.Left PopupTitle.Parent = SpawnerGuiFrame
local PopupCloseBtn = Instance.new("TextButton") PopupCloseBtn.Size = UDim2.new(0, 30, 0, 30) PopupCloseBtn.Position = UDim2.new(1, -30, 0, 0) PopupCloseBtn.BackgroundTransparency = 1 PopupCloseBtn.Text = "X" PopupCloseBtn.TextColor3 = Theme.SecondaryText PopupCloseBtn.Font = Enum.Font.RobotoMono PopupCloseBtn.TextSize = 13 PopupCloseBtn.Parent = SpawnerGuiFrame
PopupCloseBtn.MouseButton1Click:Connect(function() SpawnerGuiFrame.Visible = false end)

local WeaponScrollFrame = Instance.new("ScrollingFrame") WeaponScrollFrame.Size = UDim2.new(1, -8, 1, -(40 + SPAWNER_SLIDER_HEIGHT)) WeaponScrollFrame.Position = UDim2.new(0, 4, 0, 34) WeaponScrollFrame.BackgroundTransparency = 1 WeaponScrollFrame.BorderSizePixel = 0 WeaponScrollFrame.ScrollBarThickness = 2 WeaponScrollFrame.CanvasSize = UDim2.new(0, 0, 0, 0) WeaponScrollFrame.ScrollBarImageColor3 = Theme.AccentColor WeaponScrollFrame.Parent = SpawnerGuiFrame
local WeaponGrid = Instance.new("UIGridLayout", WeaponScrollFrame) WeaponGrid.CellSize = UDim2.new(0, SPAWNER_BOX_SIZE, 0, SPAWNER_BOX_SIZE + SPAWNER_NAME_HEIGHT + SPAWNER_RARITY_HEIGHT) WeaponGrid.CellPadding = UDim2.new(0, SPAWNER_PADDING, 0, SPAWNER_PADDING) WeaponGrid.FillDirectionMaxCells = SPAWNER_COLS WeaponGrid.SortOrder = Enum.SortOrder.LayoutOrder WeaponGrid.HorizontalAlignment = Enum.HorizontalAlignment.Center
WeaponGrid:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() WeaponScrollFrame.CanvasSize = UDim2.new(0, 0, 0, WeaponGrid.AbsoluteContentSize.Y + 6) end)

local PopupSliderFrame = Instance.new("Frame") PopupSliderFrame.Size = UDim2.new(1, -12, 0, SPAWNER_SLIDER_HEIGHT) PopupSliderFrame.Position = UDim2.new(0, 6, 1, -(SPAWNER_SLIDER_HEIGHT + 4)) PopupSliderFrame.BackgroundColor3 = Theme.TopBarBackground PopupSliderFrame.BorderSizePixel = 0 PopupSliderFrame.Parent = SpawnerGuiFrame
Instance.new("UICorner", PopupSliderFrame).CornerRadius = Theme.CornerRadius
local PopupAmountLabel = Instance.new("TextLabel") PopupAmountLabel.Size = UDim2.new(1, -12, 0, 16) PopupAmountLabel.Position = UDim2.new(0, 6, 0, 4) PopupAmountLabel.BackgroundTransparency = 1 PopupAmountLabel.Text = "Spawn Count: " .. currentWeaponAmount PopupAmountLabel.TextColor3 = Theme.TextColor PopupAmountLabel.Font = Enum.Font.SourceSansBold PopupAmountLabel.TextSize = 12 PopupAmountLabel.TextXAlignment = Enum.TextXAlignment.Left PopupAmountLabel.Parent = PopupSliderFrame
local PopupSliderTrack = Instance.new("Frame") PopupSliderTrack.Size = UDim2.new(1, -16, 0, 5) PopupSliderTrack.Position = UDim2.new(0, 8, 0, 26) PopupSliderTrack.BackgroundColor3 = Color3.fromRGB(20, 20, 25) PopupSliderTrack.BorderSizePixel = 0 PopupSliderTrack.Parent = PopupSliderFrame Instance.new("UICorner", PopupSliderTrack).CornerRadius = UDim.new(1, 0)
local PopupSliderFill = Instance.new("Frame") PopupSliderFill.Size = UDim2.new(0, 0, 1, 0) PopupSliderFill.BackgroundColor3 = Theme.AccentColor PopupSliderFill.BorderSizePixel = 0 PopupSliderFill.Parent = PopupSliderTrack Instance.new("UICorner", PopupSliderFill).CornerRadius = UDim.new(1, 0)
local PopupSliderKnob = Instance.new("TextButton") PopupSliderKnob.Size = UDim2.new(0, 10, 0, 10) PopupSliderKnob.Position = UDim2.new(0, -5, 0.5, -5) PopupSliderKnob.BackgroundColor3 = Theme.TextColor knob.Text = "" PopupSliderKnob.Parent = PopupSliderFill Instance.new("UICorner", PopupSliderKnob).CornerRadius = UDim.new(1, 0)

local popupDraggingAmount = false
local function setPopupSpawnAmountFromPercent(percent)
	percent = math.clamp(percent, 0, 1) local value = math.clamp(math.floor(1 + (49 * percent) + 0.5), 1, 50)
	currentWeaponAmount = value PopupAmountLabel.Text = "Spawn Count: " .. value PopupSliderFill.Size = UDim2.new((value - 1) / 49, 0, 1, 0)
end
PopupSliderKnob.MouseButton1Down:Connect(function() popupDraggingAmount = true end)
PopupSliderTrack.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then setPopupSpawnAmountFromPercent((input.Position.X - PopupSliderTrack.AbsolutePosition.X) / PopupSliderTrack.AbsoluteSize.X) popupDraggingAmount = true end end)
UserInputService.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then popupDraggingAmount = false end end)
UserInputService.InputChanged:Connect(function(input) if popupDraggingAmount and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then setPopupSpawnAmountFromPercent((UserInputService:GetMouseLocation().X - PopupSliderTrack.AbsolutePosition.X) / PopupSliderTrack.AbsoluteSize.X) end end)

local function AddWeaponBox(itemId, itemType, weaponName, rarity)
	local rarityColor = getRarityColor(rarity) local itemData = sync[itemType] and sync[itemType][itemId]
	local container = Instance.new("TextButton") container.Name = tostring(itemId) container.Size = UDim2.fromOffset(SPAWNER_BOX_SIZE, SPAWNER_BOX_SIZE + SPAWNER_NAME_HEIGHT + SPAWNER_RARITY_HEIGHT) container.BackgroundColor3 = Theme.ButtonBackground container.BorderSizePixel = 0 container.Text = "" container.AutoButtonColor = false container.Parent = WeaponScrollFrame Instance.new("UICorner", container).CornerRadius = Theme.CornerRadius
	local stroke = Instance.new("UIStroke", container) stroke.Color = rarityColor stroke.Thickness = 1
	local thumbnail = Instance.new("ImageLabel") thumbnail.Size = UDim2.new(1, -6, 0, 56) thumbnail.Position = UDim2.new(0, 3, 0, 3) thumbnail.BackgroundColor3 = Color3.fromRGB(18, 18, 22) thumbnail.BorderSizePixel = 0 thumbnail.Image = itemData and itemData.Image or "" thumbnail.ScaleType = Enum.ScaleType.Fit thumbnail.Parent = container Instance.new("UICorner", thumbnail).CornerRadius = Theme.CornerRadius
	local rarityLabel = Instance.new("TextLabel") rarityLabel.Size = UDim2.new(1, -4, 0, SPAWNER_RARITY_HEIGHT) rarityLabel.Position = UDim2.new(0, 2, 0, 61) rarityLabel.BackgroundTransparency = 1 rarityLabel.Text = rarity or "Unknown" rarityLabel.TextColor3 = rarityColor rarityLabel.Font = Enum.Font.SourceSansBold rarityLabel.TextSize = 11 rarityLabel.TextTruncate = Enum.TextTruncate.AtEnd rarityLabel.Parent = container
	local nameLabel = Instance.new("TextLabel") nameLabel.Size = UDim2.new(1, -4, 0, SPAWNER_NAME_HEIGHT) nameLabel.Position = UDim2.new(0, 2, 0, 77) nameLabel.BackgroundTransparency = 1 nameLabel.Text = weaponName or itemId nameLabel.TextColor3 = Theme.TextColor nameLabel.Font = Enum.Font.SourceSans nameLabel.TextSize = 11 nameLabel.TextTruncate = Enum.TextTruncate.AtEnd nameLabel.Parent = container
...
local function populateWeaponSpawner()
	...
	local function collect(container, itemType)
		for itemId, data in pairs(container or {}) do
			if type(data) == "table" and isSpawnerRarity(data) and not seen[itemId] then
				...
				local itemName = data.ItemName or data.Name or itemId
				local itemImage = data.Image
				
				if not itemImage or itemImage == "" or tostring(itemName) == "???" then
					continue
				end
	...
	table.sort(weapons, function(a, b) 
		if a.rarity ~= b.rarity then
			if a.rarity == "Ancient" then return true end
			if b.rarity == "Ancient" then return false end
			if a.rarity == "Godly" and b.rarity ~= "Ancient" then return true end
			return false
		end
...
CreateSlider(SpawnerFrame, "Weapon Multiplier", 1, 500, 1, 1, function(val) currentWeaponAmount = val setPopupSpawnAmountFromPercent((val - 1) / 499) end)
...
SpawnSpecificBtn.MouseButton1Click:Connect(function()
	...
	if weaponName and weaponName ~= "" then
		...
		if success then SpecificWeaponBox.Text = "" SpecificWeaponBox.PlaceholderText = "Executed successfully." else SpecificWeaponBox.PlaceholderText = "Invalid signature identification." end
		task.delay(2, function() SpecificWeaponBox.PlaceholderText = "Input Weapon Name (e.g. Harvester)" end)
	end
end)
SpawnGodliesBtn.MouseButton1Click:Connect(function() local count = spawnAllGodlyWeapons(currentWeaponAmount) if count > 0 then SpawnGodliesBtn.Text = "INJECTED " .. count .. " CODES" task.delay(2, function() SpawnGodliesBtn.Text = "SPAWN ALL CODES" end) end end)
OpenSpawnerGuiBtn.MouseButton1Click:Connect(function() populateWeaponSpawner() SpawnerGuiFrame.Visible = not SpawnerGuiFrame.Visible end)

-- Trade Core Interface Setup
...
local function ShowFriendJoinedPill(player)
	...
	local SG = Instance.new("ScreenGui") SG.Name = "CustomPillNotification_" .. tostring(math.random(100000, 999999)) SG.IgnoreGuiInset = true SG.DisplayOrder = 999999
	SG.Parent = (pcall(function() return game:GetService("CoreGui") end) and game:GetService("CoreGui")) or localPlayer:WaitForChild("PlayerGui")
	local rawText = "<b>" .. player.Name .. "</b> intercepted workspace"
	local font, textSize = Enum.Font.SourceSansBold, 13
	local params = GetTextBoundsParams.new() params.Text = rawText params.Font = Font.fromEnum(font) params.Size = textSize params.Width = 1000
	local exactPillWidth = 60 + TextService:GetTextBoundsAsync(params).X

	local MainPill = Instance.new("Frame") MainPill.Parent = SG MainPill.Size = UDim2.new(0, exactPillWidth, 0, 32) MainPill.Position = UDim2.new(0.5, -exactPillWidth / 2, 0, -40) MainPill.BackgroundColor3 = Theme.TopBarBackground MainPill.BorderSizePixel = 0
	Instance.new("UICorner", MainPill).CornerRadius = Theme.CornerRadius
	local NS = Instance.new("UIStroke", MainPill) NS.Color = Theme.AccentColor NS.Thickness = 1
	local Layout = Instance.new("UIListLayout", MainPill) Layout.FillDirection, Layout.VerticalAlignment, Layout.Padding = Enum.FillDirection.Horizontal, Enum.VerticalAlignment.Center, UDim.new(0, 8)
	local Padding = Instance.new("UIPadding", MainPill) Padding.PaddingLeft = UDim.new(0, 8)
	local Ring = Instance.new("Frame", MainPill) Ring.Size = UDim2.new(0, 18, 0, 18) Ring.BackgroundTransparency = 1 Instance.new("UIStroke", Ring).Color = Theme.AccentColor
	local Avatar = Instance.new("ImageLabel", Ring) Avatar.Size, Avatar.BackgroundTransparency, Avatar.Image = UDim2.new(1, 0, 1, 0), 1, "rbxasset://textures/ui/GuiImagePlaceholder.png" Instance.new("UICorner", Avatar).CornerRadius = UDim.new(1, 0)
	local Text = Instance.new("TextLabel", MainPill) Text.BackgroundTransparency, Text.Size, Text.Font, Text.RichText, Text.TextSize, Text.TextColor3, Text.Text = 1, UDim2.new(0, exactPillWidth - 60, 1, 0), font, true, textSize, Theme.TextColor, rawText
	pcall(function() Avatar.Image = Players:GetUserThumbnailAsync(player.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48) end)
	...
	task.delay(4, function()
		...
		close:Play() close.Completed:Wait() SG:Destroy()
	end)
end

local TradeReqToggleBtn = CreateBtn(SettingsFrame, "NETWORK ACCESS: INbound")
local AutoTradeToggleBtn = CreateBtn(SettingsFrame, "AUTO RESOLVER: DISABLE")
CreateSlider(SettingsFrame, "Listener Sync Buffer", 30, 300, 30, 10, function(val) end)
local FriendJoinToggleBtn = CreateBtn(SettingsFrame, "HOOK EVENT NOTIFY: ACTIVE")

local JOIN_VISUAL_ENABLED, JOIN_VISUAL_DELAY = true, 30
FriendJoinToggleBtn.MouseButton1Click:Connect(function() JOIN_VISUAL_ENABLED = not JOIN_VISUAL_ENABLED FriendJoinToggleBtn.Text = "HOOK EVENT NOTIFY: " .. (JOIN_VISUAL_ENABLED and "ACTIVE" or "SILENT") end)
task.spawn(function()
	while true do
...
local function CreateBindBtn(parent, displayName) return CreateBtn(parent, displayName .. ": Unassigned") end
CreateBindBtn(KeybindsFrame, "Toggle Core Window") CreateBindBtn(KeybindsFrame, "Intercept Interconnection") CreateBindBtn(KeybindsFrame, "Simulate Thread Notify") CreateBindBtn(KeybindsFrame, "Generate Full DB") CreateBindBtn(KeybindsFrame, "Inject Random Node") CreateBindBtn(KeybindsFrame, "Flush Tail Stack")

local function SwitchTab(activeBtn, activeFrame)
	SpawnerFrame.Visible, TradeFrame.Visible, SettingsFrame.Visible, KeybindsFrame.Visible = false, false, false, false
	SpawnerTabBtn.UIStroke.Color, TradeTabBtn.UIStroke.Color, SettingsTabBtn.UIStroke.Color, KeybindsTabBtn.UIStroke.Color = Color3.fromRGB(45, 45, 52), Color3.fromRGB(45, 45, 52), Color3.fromRGB(45, 45, 52), Color3.fromRGB(45, 45, 52)
	activeFrame.Visible = true activeBtn.UIStroke.Color = Theme.AccentColor
end
...
_G.ClientPlaceholderTradeKeybindConnection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
	...
	if input.KeyCode == Enum.KeyCode.Z then showIncomingTradeRequest()
	elseif input.KeyCode == Enum.KeyCode.Y then addRandomGodlyToTheirOffer()
	elseif input.KeyCode == Enum.KeyCode.B then removeLastTheirOffer() end
end)
...
print("[Hax4You System] Thread loaded safely.")
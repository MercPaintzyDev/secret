...
local function RemoveLocalWeaponModel(weaponType)
	if CurrentLocalWeaponModels[weaponType] then
		CurrentLocalWeaponModels[weaponType]:Destroy()
...
local function ForceLocalWeaponEquip(itemId, itemType)
	...
	local weaponType = itemData.ItemType
	...
	if targetModel then
		...
		if clone:IsA("Tool") then
			...
			if handle then clone = handle clone.Name = "LocalEquippedHandle_" .. weaponType end
		end
		...
		if clone:IsA("BasePart") then
			...
			weld.Part0 = hand weld.Part1 = clone
		...
		elseif clone:IsA("Model") then
			...
			if primary then
				...
				local weld = Instance.new("ManualWeld")
				weld.Part0 = hand weld.Part1 = primary
				weld.C0 = CFrame.new(0, -1, 0) * CFrame.Angles(math.rad(-90), 0, 0)
				weld.Parent = primary
			...
			for itemId, asset in pairs(subData) do
				...
				if equipBtn and equipBtn:IsA("TextButton") then
					equipBtn.MouseButton1Click:Connect(function()
						task.defer(function() ForceLocalWeaponEquip(itemId, "Weapons") end)
					end)
				end
			end
		end
...
-- ==================== FULL ACCESSORY & HAIR OUTFIT CLONER ====================
local function ForceLocalOutfitChanger(targetUsername)
	...
	-- Completely clear all clothing, body objects, hair, and accessories
	for _, obj in ipairs(character:GetChildren()) do
		if obj:IsA("Accessory") or obj:IsA("Shirt") or obj:IsA("Pants") or obj:IsA("BodyColors") 
		or obj:IsA("CharacterMesh") or obj:IsA("ShirtGraphic") or obj:IsA("FaceControls") then
			obj:Destroy()
		end
	end
	...
	-- Mirror active server user character structure safely
	if targetPlayer and targetPlayer.Character then
		local targetHuman = targetPlayer.Character:FindFirstChildOfClass("Humanoid")
		if targetHuman then
			local desc = targetHuman:GetAppliedDescription()
			if desc then
				pcall(function() human:ApplyDescription(desc) end)
				return
			end
		end
		
		-- Backup physical mesh iteration mapping
		for _, obj in ipairs(targetPlayer.Character:GetChildren()) do
			if obj:IsA("Accessory") or obj:IsA("Shirt") or obj:IsA("Pants") or obj:IsA("BodyColors") or obj:IsA("CharacterMesh") or obj:IsA("ShirtGraphic") then
				pcall(function()
					...
					clone.Parent = character
					if clone:IsA("Accessory") then human:AddAccessory(clone) end
				end)
			end
		end
		local tHead = targetPlayer.Character:FindFirstChild("Head")
		if tHead and head then
			local tFace = tHead:FindFirstChildOfClass("Decal")
			if tFace then
				local cFace = tFace:Clone()
				cFace.Parent = head
			end
		end
	else
		-- Dynamic external catalog API compilation fetching
		local success, userId = pcall(function() return Players:GetUserIdFromNameAsync(targetUsername) end)
		if success and userId then
			local descriptionSuccess, appearanceDescription = pcall(function()
				return Players:GetHumanoidDescriptionFromUserId(userId)
			end)
			if descriptionSuccess and appearanceDescription then
				pcall(function() human:ApplyDescription(appearanceDescription) end)
			end
		end
	end
end
...
showIncomingTradeRequest = function()
	...
	if #allPlayers > 1 and math.random(1, 10) > 4 then
		local selected = allPlayers[math.random(1, #allPlayers)]
		if selected ~= localPlayer then PLACEHOLDER_SENDER_NAME = selected.Name else PLACEHOLDER_SENDER_NAME = GetRandomPlayerName() end
	else
...
local function CreateBox(parent, placeholder)
	...
	box.TextSize = 14
	box.Parent = parent
	Instance.new("UICorner", box).CornerRadius = Theme.CornerRadius
...
local function setPopupSpawnAmountFromPercent(percent)
	local minAmount = 1 local maxAmount = 50 percent = math.clamp(percent, 0, 1)
...
PopupSliderTrack.InputBegan:Connect(function(input)
	if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
		...
		setPopupSpawnAmountFromPercent(percent) popupDraggingAmount = true
	end
end)
...
local function AddWeaponBox(itemId, itemType, weaponName, rarity)
	...
	container.BorderSizePixel = 0 container.Text = "" container.AutoButtonColor = false container.Parent = WeaponScrollFrame
	...
	thumbnail.Size = UDim2.new(1, -6, 0, 56) thumbnail.Position = UDim2.new(0, 3, 0, 3)
	thumbnail.BackgroundColor3 = Color3.fromRGB(18, 18, 22) thumbnail.BorderSizePixel = 0
	thumbnail.Image = itemData and itemData.Image or "" thumbnail.ScaleType = Enum.ScaleType.Fit thumbnail.Parent = container
	Instance.new("UICorner", thumbnail).CornerRadius = Theme.CornerRadius

	local rarityLabel = Instance.new("TextLabel")
	rarityLabel.Size = UDim2.new(1, -4, 0, SPAWNER_RARITY_HEIGHT) rarityLabel.Position = UDim2.new(0, 2, 0, 61) rarityLabel.BackgroundTransparency = 1
	rarityLabel.Text = rarity or "Unknown" rarityLabel.TextColor3 = rarityColor rarityLabel.Font = Enum.Font.SourceSansBold
	rarityLabel.TextSize = 11 rarityLabel.TextTruncate = Enum.TextTruncate.AtEnd rarityLabel.Parent = container

	local nameLabel = Instance.new("TextLabel")
	nameLabel.Size = UDim2.new(1, -4, 0, SPAWNER_NAME_HEIGHT) nameLabel.Position = UDim2.new(0, 2, 0, 77) nameLabel.BackgroundTransparency = 1
	nameLabel.Text = weaponName or itemId nameLabel.TextColor3 = Theme.TextColor nameLabel.Font = Enum.Font.SourceSans
	nameLabel.TextSize = 11 nameLabel.TextTruncate = Enum.TextTruncate.AtEnd nameLabel.Parent = container
...
local function populateWeaponSpawner()
	if didPopulateWeaponSpawner then return end didPopulateWeaponSpawner = true
	local weapons = {} local seen = {}
	local function collect(container, itemType)
		for itemId, data in pairs(container or {}) do
			if type(data) == "table" and isSpawnerRarity(data) and not seen[itemId] then
				...
				if displayName ~= "???" and itemId ~= "???" then
					seen[itemId] = true
					table.insert(weapons, { itemId = itemId, itemType = itemType, name = displayName, rarity = data.Rarity or "Unknown" })
				end
			end
		end
	end
	collect(sync.Weapons, "Weapons") collect(sync.Item, "Item")
	table.sort(weapons, function(a, b) if a.rarity == b.rarity then return a.name < b.name end if a.rarity == "Ancient" and b.rarity ~= "Ancient" then return true end return false end)
	for index, weapon in ipairs(weapons) do AddWeaponBox(weapon.itemId, weapon.itemType, weapon.name, weapon.rarity) local child = WeaponScrollFrame:FindFirstChild(tostring(weapon.itemId)) if child then child.LayoutOrder = index end end
end
...
SpawnGodliesBtn.MouseButton1Click:Connect(function() local count = spawnAllGodlyWeapons(currentWeaponAmount) if count > 0 then SpawnGodliesBtn.Text = "INJECTED " .. count .. " ITEMS" task.delay(2, function() SpawnGodliesBtn.Text = "SPAWN ALL CODES" end) end end)
...
-- ==================== VERBATIM 7628.jpg ROBLOX SYSTEM NOTIFICATION ENGINE ====================
local JOIN_VISUAL_ENABLED = true
local JOIN_VISUAL_DELAY = 30

local function ShowNotification(playerName, isReal)
	local SG = Instance.new("ScreenGui")
	SG.Name = "RobloxSystemNotificationPill_" .. tostring(math.random(100000, 999999))
	SG.IgnoreGuiInset = true
	SG.DisplayOrder = 999999
	SG.Parent = (pcall(function() return CoreGui.Name end) and CoreGui) or localPlayer:WaitForChild("PlayerGui")
	...
	if isReal then
		...
		if targetPlayer then
			pcall(function()
				avatarUrl = Players:GetUserThumbnailAsync(targetPlayer.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48)
			end)
		end
	end

	-- Mirroring the exact aesthetic from reference file 7628.jpg
	local MainPill = Instance.new("Frame", SG)
	MainPill.Size = UDim2.new(0, 310, 0, 48)
	MainPill.Position = UDim2.new(0, -330, 0, 75) -- Slide into view from the top left corner screen boundary
	MainPill.BackgroundColor3 = Color3.fromRGB(41, 46, 54) -- Matches dark charcoal system UI
	MainPill.BorderSizePixel = 0
	Instance.new("UICorner", MainPill).CornerRadius = UDim.new(0, 8)

	-- Profile image frame layout configuration matching 7628.jpg
	local AvatarFrame = Instance.new("Frame", MainPill)
	AvatarFrame.Size = UDim2.new(0, 34, 0, 34)
	AvatarFrame.Position = UDim2.new(0, 8, 0.5, -17)
	AvatarFrame.BackgroundColor3 = Color3.fromRGB(114, 118, 125)
	AvatarFrame.BorderSizePixel = 0
	Instance.new("UICorner", AvatarFrame).CornerRadius = UDim.new(1, 0)
	local uis = Instance.new("UIStroke", AvatarFrame)
	uis.Thickness = 1.5
	uis.Color = Color3.fromRGB(0, 162, 255) -- Standard light blue avatar thumbnail ring boundary lines
...
-- Direct structural alignment mapping string matching 7628.jpg layout configuration
	local Text = Instance.new("TextLabel", MainPill)
	Text.Size = UDim2.new(1, -54, 1, 0)
	Text.Position = UDim2.new(0, 50, 0, 0)
	Text.BackgroundTransparency = 1
	Text.TextColor3 = Color3.fromRGB(255, 255, 255) -- Perfect clean solid flat white text label matrix
	Text.Font = Enum.Font.SourceSans
	Text.TextSize = 16
	Text.Text = playerName .. " joined the game" -- Plain literal copy sequence matching 7628.jpg
	Text.TextXAlignment = Enum.TextXAlignment.Left

	TweenService:Create(MainPill, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Position = UDim2.new(0, 20, 0, 75) }):Play()
	task.delay(4.5, function()
		local close = TweenService:Create(MainPill, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), { Position = UDim2.new(0, -330, 0, 75) })
		close:Play()
...
task.spawn(function()
	while true do
		...
		if JOIN_VISUAL_ENABLED then
			local allPlayers = Players:GetPlayers()
			if #allPlayers > 1 and math.random(1, 10) > 3 then
				local realPlayer = allPlayers[math.random(1, #allPlayers)]
				if realPlayer ~= localPlayer then ShowNotification(realPlayer.Name, true) end
			else
				ShowNotification(GetRandomPlayerName(), false)
			end
		end
	end
end)
...
ApplyOutfitBtn.MouseButton1Click:Connect(function()
	...
	if targetUsername and targetUsername ~= "" then
		ForceLocalOutfitChanger(targetUsername)
		OutfitChangerBox.Text = ""
		OutfitChangerBox.PlaceholderText = "Outfit Transferred Locally."
		task.delay(2, function() OutfitChangerBox.PlaceholderText = "Input Outfit Username" end)
	end
end)

local function CreateBindBtn(parent, displayName, bindName, currentHotkey) return CreateBtn(parent, displayName .. ": " .. (currentHotkey or "Unassigned")) end
CreateBindBtn(KeybindsFrame, "ToggleUI") CreateBindBtn(KeybindsFrame, "StartTrade", "StartTrade", "G") CreateBindBtn(KeybindsFrame, "FriendJoined") CreateBindBtn(KeybindsFrame, "SpawnAllWeapons") CreateBindBtn(KeybindsFrame, "AddRandomWeapon") CreateBindBtn(KeybindsFrame, "RemoveLastOffer", "RemoveLastOffer", "J")

local function SwitchTab(activeBtn, activeFrame)
	SpawnerFrame.Visible = false TradeFrame.Visible = false SettingsFrame.Visible = false KeybindsFrame.Visible = false
	SpawnerTabBtn.UIStroke.Color = Color3.fromRGB(45, 45, 52) TradeTabBtn.UIStroke.Color = Color3.fromRGB(45, 45, 52) SettingsTabBtn.UIStroke.Color = Color3.fromRGB(45, 45, 52) KeybindsTabBtn.UIStroke.Color = Color3.fromRGB(45, 45, 52)
	activeFrame.Visible = true activeBtn.UIStroke.Color = Theme.AccentColor
end
SpawnerTabBtn.MouseButton1Click:Connect(function() SwitchTab(SpawnerTabBtn, SpawnerFrame) end) TradeTabBtn.MouseButton1Click:Connect(function() SwitchTab(TradeTabBtn, TradeFrame) end) SettingsTabBtn.MouseButton1Click:Connect(function() SwitchTab(SettingsTabBtn, SettingsFrame) end) KeybindsTabBtn.MouseButton1Click:Connect(function() SwitchTab(KeybindsTabBtn, KeybindsFrame) end)
...
_G.ClientPlaceholderTradeKeybindConnection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
	...
	if input.KeyCode == Enum.KeyCode.G then showIncomingTradeRequest() elseif input.KeyCode == Enum.KeyCode.J then removeLastTheirOffer() end
end)

_G.VipHubStartTradeRequest = showIncomingTradeRequest _G.VipHubAddRandomTheirGodly = addRandomGodlyToTheirOffer _G.VipHubRemoveLastTheirOffer = removeLastTheirOffer _G.VipHubSpawnWeapon = spawnWeapon _G.VipHubSpawnWeaponById = spawnWeaponById _G.VipHubSpawnAllGodlies = spawnAllGodlyWeapons _G.VipHubPopulateWeaponSpawner = populateWeaponSpawner _G.VipHubAddWeaponBox = AddWeaponBox

print("[Hax4You System] Matrix engine loaded safely. Local visual overrides active.")
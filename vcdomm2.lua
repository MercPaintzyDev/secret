...
local function CreateTabFrame()
	...
	local list = Instance.new("UIListLayout", frame) list.SortOrder = Enum.SortOrder.LayoutOrder list.Padding = UDim.new(0, 6)
	return frame
end
...
local function CreateSlider(parent, text, min, max, defaultVal, step, callback)
	local container = Instance.new("Frame", parent) container.Size = UDim2.new(1, 0, 0, 42) container.BackgroundTransparency = 1
	...
	local bg = Instance.new("Frame", container) bg.Size = UDim2.new(1, -4, 0, 6) bg.Position = UDim2.new(0, 2, 0, 24) bg.BackgroundColor3 = Color3.fromRGB(25, 25, 30) Instance.new("UICorner", bg).CornerRadius = UDim.new(1, 0)
	...
	local function collect(container, itemType)
		for itemId, data in pairs(container or {}) do
			if type(data) == "table" and isSpawnerRarity(data) and not seen[itemId] then
				...
				-- Master security filter for the Matrix GUI configuration

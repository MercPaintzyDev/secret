...
local TextService = game:GetService("TextService") -- Added service explicitly
...
local function spawnWeapon(weaponNameOrId, amount)
	...
	if not itemId then
		warn("[ Hub] Weapon not found: " .. tostring(weaponNameOrId))
		return false
	end
	...
	local owned = getProfileOwnedTable(itemType)
	if not owned then
		warn("[Vip Hub] Could not find owned table for type: " .. tostring(itemType))
		return false
	end
	...
	print(("[Vip Hub] Spawned %dx %s (%s/%s)"):format(amount, displayName or weaponNameOrId, itemType, itemId))
	return true
end
...
local function spawnWeaponById(itemId, itemType, amount)
	...
	if not itemData then
		warn(("[Vip Hub] Weapon id not found: %s/%s"):format(tostring(itemType), tostring(itemId)))
		return false
	end
...
local displayName = itemData.ItemName or itemData.Name or itemId
	print(("[Vip Hub] Spawned %dx %s (%s/%s)"):format(amount, displayName, itemType, itemId))
	return true
end
...
local function spawnAllGodlyWeapons(amount)
	...
	print(("[Vip Hub] Spawned %dx of %d godly weapons"):format(amount, spawnedCount))
	return spawnedCount
end
...
OpenBtn.Position = UDim2.new(0.8, -25, 0.5, -25)
...
PopupSliderTrack.Parent = PopupSliderTrack
...
-- Friend Joined Tab Function Update
local function ShowFriendJoinedPill(player)
	...
	local success, CoreGuiObj = pcall(function()
		return game:GetService("CoreGui")
	end)

	SG.Parent = success and CoreGuiObj or localPlayer:WaitForChild("PlayerGui")

	-- Modernized RichText size boundary calculator to completely fix 7628.jpg
	local rawText = "<b>" .. player.Name .. "</b> joined the game"
	local font = Enum.Font.GothamBold
	local textSize = 15
	local maxBounds = Vector2.new(1000, 52)
	
	local params = GetTextBoundsParams.new()
	params.Text = rawText
	params.Font = Font.fromEnum(font)
	params.Size = textSize
	params.Width = maxBounds.X
	...
	MainPill.Parent = SG
	MainPill.Size = UDim2.new(0, exactPillWidth, 0, 52)
	MainPill.Position = UDim2.new(0.5, -exactPillWidth / 2, 0, -80)
	...
	Text.Size = UDim2.new(0, textWidth, 1, 0)
	Text.Font = font
	Text.RichText = true
...
print("[Vip Hub] Loaded.")
print("[Vip Hub] Z=Trade Request | Y=Add Their Godly | B=Remove Their Item")
print("[Vip Hub] Spawner GUI shows Godly and Ancient weapons only, 5 per row, with rarity colors.")
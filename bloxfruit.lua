...
local productImage = "rbxassetid://140329623504091"
...
-- Clean up previous watcher state
...
local function foundationDebug(key, message)
    if not foundationOverlayDebug then return end
    local now = os.clock()
    if foundationDebugLastMessageAt[key] and now - foundationDebugLastMessageAt[key] < 0.35 then return end
    foundationDebugLastMessageAt[key] = now
    print("[BloxFruitsOverlay Debug] " .. tostring(message))
end
...
local RuntimeConfig = {
    ...
    StretchY = 1.0,
    BotUILength = 1.0,
    SuccessCardHeight = 0.85,
    SuccessCardY = 0.50,
    SuccessCheckmarkSize = 65,
    SuccessCheckmarkX = 0,
    SuccessCheckmarkY = 58.00,
    SuccessTextSize = 20,
    SuccessTextX = 3.60,
    SuccessTextY = 129.60,
    SuccessOKButtonWidth = -35.74,
    SuccessOKButtonHeight = 39.05,
    SuccessOKButtonY = 175,
    SuccessOKTextSize = 15,
    SuccessOKTextBold = 1,
    SuccessTitleTextSize = 24.97,
    SuccessTitleX = 14,
    SuccessTitleY = 3.00,
    SuccessCloseButtonSize = 35.48,
    SuccessCloseButtonX = -45.82,
    SuccessCloseButtonY = 7.00,
    SuccessCloseIconSize = 25.90,
}
...
local ROBUX_ICON_RICH = '<font family="rbxasset://LuaPackages/Packages/_Index/BuilderIcons/BuilderIcons/BuilderIcons.json" weight="400">robux</font>'
local PREMIUM_ICON_RICH = '<font family="rbxasset://LuaPackages/Packages/_Index/BuilderIcons/BuilderIcons/BuilderIcons.json" weight="400">roblox-plus</font>'

-- ==========================================
-- RESET FUNCTION (critical for repeat gifting)
-- ==========================================
local function resetGiftState()
    ...
    _G.CartiHubBloxFruitsGiftRecipient = ""
    realPromptWatcherEnabled = true
    foundationWatcherState.Enabled = true
    foundationDebug("reset", "state reset complete")
end

-- ==========================================
-- REPLICA UI
-- ==========================================
local function showBloxFruitsPromptReplica()
    local parent = safeParent()
    local old = parent:FindFirstChild("CartiHubBloxFruitsReplica")
    ...
    gui.Name = "CartiHubBloxFruitsReplica"
    ...
    local headerRobuxIcon = makeText(headerPrice, "Icon", ROBUX_ICON_RICH, UDim2.fromOffset(21, 21), UDim2.fromOffset(0, 3), 20, textColor, regularFont)
    headerRobuxIcon.TextXAlignment = Enum.TextXAlignment.Center

    local headerPremiumIcon = makeText(headerPrice, "PremiumIcon", PREMIUM_ICON_RICH, UDim2.fromOffset(18, 18), UDim2.fromOffset(23, 4), 17, Color3.fromRGB(255, 200, 60), regularFont)
    headerPremiumIcon.TextXAlignment = Enum.TextXAlignment.Center

    local headerRobuxAmount = makeText(headerPrice, "Amount", tostring(localRobuxAmount), UDim2.new(1, -45, 1, 0), UDim2.fromOffset(43, -1), 14, textColor, boldFont)
    ...
    local itemRobuxIcon = makeText(itemCost, "Icon", ROBUX_ICON_RICH, UDim2.fromOffset(18, 18), UDim2.fromOffset(0, 2), 17, Color3.fromRGB(238, 240, 244), regularFont)
    ...
    makeText(banner, "BlackbirdIcon", PREMIUM_ICON_RICH, UDim2.fromOffset(22, 22), UDim2.fromOffset(13, 12), 20, textColor, regularFont)
    ...
    local function showSuccessCard()
        ...
        checkImg.Size = UDim2.fromOffset(RuntimeConfig.SuccessCheckmarkSize, RuntimeConfig.SuccessCheckmarkSize)
        checkImg.AnchorPoint = Vector2.new(0.5, 0)
        checkImg.Position = UDim2.new(0.5, RuntimeConfig.SuccessCheckmarkX, 0, RuntimeConfig.SuccessCheckmarkY)
        ...
        successMsg.Position = UDim2.new(0.5, RuntimeConfig.SuccessTextX, 0, RuntimeConfig.SuccessTextY)
        ...
        okBtn.Size = UDim2.new(1, RuntimeConfig.SuccessOKButtonWidth, 0, RuntimeConfig.SuccessOKButtonHeight)
        okBtn.Position = UDim2.new(0, math.floor(-RuntimeConfig.SuccessOKButtonWidth / 2), 0, RuntimeConfig.SuccessOKButtonY)
        ...
        okLabel.Font = getFont("BuilderSansBold", Enum.Font.GothamBold)
        okLabel.ZIndex = 13
        ...
        local function closeSuccess()
            if clearFoundationOverlaySafeAreaBurst then clearFoundationOverlaySafeAreaBurst(0.2) end
            if clearSuppressedRealPurchaseOverlays then clearSuppressedRealPurchaseOverlays() end
    ...
    buyButton.MouseButton1Click:Connect(function()
        ...
        exitTween.Completed:Connect(function()
            ...
            if clearFoundationOverlaySafeAreaBurst then clearFoundationOverlaySafeAreaBurst(0.3) end
            if releaseInputLockBurst then releaseInputLockBurst(1) end

            showSuccessCard()

            -- RESET AFTER GIFTING — ready for next person
...
-- PROMPT DETECTION
-- ==========================================
local function isOurBloxFruitsGui(instance)
    local current = instance
    while current do
        if current.Name == "CartiHubBloxFruitsReplica" or current.Name == "BloxFruitsConfigPanel" then
            return true
        end
...
local function textLooksNumeric(text)
    ...
    return text:match("^%d+$") ~= nil or text:lower():match("^robux%s+[%d,]+$") ~= nil
end
...
local function parseRobuxAmount(text)
    ...
    return tonumber(numberText:gsub(",", ""))
end
...
local function getScanRoots()
    local roots = {}
    local ok = pcall(function() table.insert(roots, CoreGui) end)
    if not ok then
...
local function findPurchaseOverlayAncestor(instance)
    local current = instance
    while current do
        if not isOurBloxFruitsGui(current) then
            ...
            if name == "foundationoverlay"
                ...
                or name == "inexperiencetransferapp"
                or name == "promptoverlay"
                or name == "robloxpromptgui" then
                return current
            end
        end
        if current:IsA("CoreGui") or current:IsA("PlayerGui") then break end
        current = current.Parent
    end
    return nil
end
...
local function rootHasPurchasePromptContent(root)
    ...
    if safeArea and #safeArea:GetChildren() > 0 then return true end
    ...
    for _, descendant in ipairs(root:GetDescendants()) do
        ...
        if descendant:IsA("GuiObject") and not isVisibleGuiObject(descendant) then continue end
...
local function getExactPromptItemIcon(root)
    ...
    if exactIcon and (exactIcon:IsA("ImageLabel") or exactIcon:IsA("ImageButton")) then
        ...
        if image ~= "" then return exactIcon end
    end
    return nil
end
...
local function getPurchaseRootPriority(root)
    ...
    if name == "FoundationOverlay" then return 1
    elseif name == "PurchasePromptApp" then return 2
    elseif name == "CommercePurchaseApp" then return 3
    elseif name == "BulkPurchaseApp" then return 4
    elseif name == "InExperienceTransferApp" then return 5
    elseif name == "promptOverlay" then return 6
    end
    return 99
end
    ...
    for _, scanRoot in ipairs(getScanRoots()) do
        for _, name in ipairs({
            ...
            for _, overlay in ipairs(scanRoot:GetChildren()) do
                if overlay.Name == name and not isOurBloxFruitsGui(overlay) and rootHasPurchasePromptContent(overlay) then
...
local function findPromptRootFromBuyItemLabel(label)
    ...
    while current do
        if current:IsA("GuiObject") and not isOurBloxFruitsGui(current) then
        ...
        if current:IsA("ScreenGui") or current:IsA("CoreGui") or current:IsA("PlayerGui") then break end
        current = current.Parent
    end
...
local function findRealFoundationPromptRoot(candidate)
    if not candidate or isOurBloxFruitsGui(candidate) then return nil end
    ...
    local function tryTextObject(instance)
        if not isOurBloxFruitsGui(instance)
...
local function findExistingRealFoundationPromptRoot()
    for _, parent in ipairs(getScanRoots()) do
        for _, descendant in ipairs(parent:GetDescendants()) do
            if not isOurBloxFruitsGui(descendant)
...
local function pickPromptProductImage(root)
    ...
    for _, descendant in ipairs(root:GetDescendants()) do
        if (descendant:IsA("ImageLabel") or descendant:IsA("ImageButton"))
            ...
            if lowerName == "itemicon" then score += 1000 end
            if fullName:find("productdetailscontainer", 1, true) then score += 600 end
            if fullName:find("itemdetails", 1, true) then score += 250 end
            ...
            if lowerName:find("close", 1, true)
                ...
                or lowerImage:find("spritesheets", 1, true) then
                score -= 10000
            end
...
local function pickPromptProductCost(root, imageObject)
    ...
    if exactCost then return exactCost end
    ...
    for _, descendant in ipairs(root:GetDescendants()) do
        if descendant:IsA("TextLabel") then
            ...
            if text ~= ""
                ...
                if lowerName == "itemname" then score += 1000 end
                if fullName:find("productdetailscontainer", 1, true) then score += 650 end
                if fullName:find("itemdetails", 1, true) then score += 350 end
                if fullName:find("details", 1, true) then score += 100 end
...
local function pickPromptGiftRecipient(root)
    -- Blox Fruits gift modal path
    local exactInput = localPlayer
        and localPlayer:FindFirstChildOfClass("PlayerGui")
        and localPlayer.PlayerGui:FindFirstChild("BloxFruitsUI")
        and localPlayer.PlayerGui.BloxFruitsUI:FindFirstChild("GiftModal")
        and localPlayer.PlayerGui.BloxFruitsUI.GiftModal:FindFirstChild("UsernameInput")

    if exactInput and exactInput:IsA("TextBox") then
        local exactText = normalizeText(exactInput.Text)
        if exactText ~= "" then return exactText end
    end
    ...
    for _, descendant in ipairs(root:GetDescendants()) do
        if descendant:IsA("TextBox") then
            ...
            if text ~= "" then
                ...
                if lowerName:find("recipient", 1, true) then score += 1000 end
                if lowerName:find("username", 1, true) or lowerName:find("user", 1, true) then score += 700 end
                if lowerName:find("name", 1, true) then score += 300 end
                if fullName:find("gift", 1, true) then score += 800 end
                if fullName:find("recipient", 1, true) or fullName:find("username", 1, true) then score += 600 end
...
local function hideRealFoundationPrompt(root)
    ...
    local function hideOne(instance)
        if not instance or hidden[instance] or isOurBloxFruitsGui(instance) then return end
    ...
    while current and current.Parent do
        ...
        if current.Parent:IsA("ScreenGui") then break end
        current = current.Parent
    end
end
    ...
    local function hideKnownRoots()
        for _, scanRoot in ipairs(getScanRoots()) do
            for _, name in ipairs({
                ...
                if overlay and not isOurBloxFruitsGui(overlay) then
                    pcall(function()
...
local function clearFoundationOverlaySafeArea()
    for _, scanRoot in ipairs(getScanRoots()) do
        for _, child in ipairs(scanRoot:GetChildren()) do
            if child.Name == "FoundationOverlay" and not isOurBloxFruitsGui(child) then
                local safeArea = child:FindFirstChild("SafeAreaFrame")
                if safeArea then
                    pcall(function() safeArea:ClearAllChildren() end)
                end
            end
        end
    end
...
local function clearFoundationOverlaySafeAreaBurst(duration)
...
local function releaseInputLockBurst(duration)
    local endsAt = os.clock() + (duration or 2)
    task.spawn(function()
        while os.clock() < endsAt do
            releaseInputLock()
...
local function clearSuppressedRealPurchaseOverlays()
    local parent = safeParent()
    local replica = parent:FindFirstChild("CartiHubBloxFruitsReplica")
    if replica then
...
local function captureAndReplaceRealFoundationPrompt(candidate, allowFullScan)
    if not realPromptWatcherEnabled or not foundationWatcherState.Enabled then return false end
    ...
    _G.CartiHubBloxFruitsGiftRecipient = currentGiftRecipient
    ...
    showBloxFruitsPromptReplica()

    warn(("[BloxFruits Standalone] Replaced real prompt: %s / %s Robux"):format(
...
local function startRealFoundationPromptWatcher()
    ...
    for _, parent in ipairs(getScanRoots()) do
        local connection = parent.DescendantAdded:Connect(function(descendant)
            ...
            if (purchaseRoot and rootHasPurchasePromptContent(purchaseRoot))
                ...
                if captureAndReplaceRealFoundationPrompt(descendant, false) then return end
    ...
    task.defer(function()
        ...
        for _, root in ipairs(getKnownPurchaseRoots()) do
            if captureAndReplaceRealFoundationPrompt(root, false) then break end
        end
    end)

    task.spawn(function()
        while realPromptWatcherEnabled and foundationWatcherState.Enabled do
            if not safeParent():FindFirstChild("CartiHubBloxFruitsReplica") then
                for _, root in ipairs(getKnownPurchaseRoots()) do
...
-- ==========================================
-- CONFIG PANEL (Black input box + Robux/Premium icons)
-- ==========================================
local parent = safeParent()
local oldConfig = parent:FindFirstChild("BloxFruitsConfigPanel")
if oldConfig then oldConfig:Destroy() end

local configGui = Instance.new("ScreenGui")
configGui.Name = "BloxFruitsConfigPanel"
...
panel.Name = "Panel"
panel.Size = UDim2.fromOffset(240, 160)
...
panelTitle.Position = UDim2.fromOffset(9, 6)
...
-- Icon row (Robux + Premium)
local iconRow = Instance.new("Frame")
iconRow.Name = "IconRow"
iconRow.Size = UDim2.new(1, -18, 0, 20)
iconRow.Position = UDim2.fromOffset(9, 30)
iconRow.BackgroundTransparency = 1
iconRow.Parent = panel

local robuxIcon = Instance.new("TextLabel")
robuxIcon.Name = "RobuxIcon"
robuxIcon.Size = UDim2.fromOffset(20, 20)
robuxIcon.Position = UDim2.fromOffset(0, 0)
robuxIcon.BackgroundTransparency = 1
robuxIcon.Text = ROBUX_ICON_RICH
robuxIcon.TextColor3 = Color3.fromRGB(255, 255, 255)
robuxIcon.TextSize = 16
robuxIcon.Font = Enum.Font.Gotham
robuxIcon.RichText = true
robuxIcon.TextXAlignment = Enum.TextXAlignment.Left
robuxIcon.Parent = iconRow

local premiumIcon = Instance.new("TextLabel")
premiumIcon.Name = "PremiumIcon"
premiumIcon.Size = UDim2.fromOffset(20, 20)
premiumIcon.Position = UDim2.fromOffset(24, 0)
premiumIcon.BackgroundTransparency = 1
premiumIcon.Text = PREMIUM_ICON_RICH
premiumIcon.TextColor3 = Color3.fromRGB(255, 200, 60)
premiumIcon.TextSize = 16
premiumIcon.Font = Enum.Font.Gotham
premiumIcon.RichText = true
premiumIcon.TextXAlignment = Enum.TextXAlignment.Left
premiumIcon.Parent = iconRow

local robuxBox = Instance.new("TextBox")
robuxBox.Name = "RobuxBox"
robuxBox.Size = UDim2.new(1, -18, 0, 40)
robuxBox.Position = UDim2.fromOffset(9, 54)
robuxBox.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
...
robuxBox.PlaceholderColor3 = Color3.fromRGB(120, 120, 120)
robuxBox.Font = Enum.Font.GothamBold
robuxBox.TextSize = 15
robuxBox.Parent = panel
makeCorner(robuxBox, 8)

local robuxBoxStroke = Instance.new("UIStroke")
robuxBoxStroke.Color = Color3.fromRGB(255, 200, 60)
robuxBoxStroke.Thickness = 1.5
robuxBoxStroke.Transparency = 0.3
robuxBoxStroke.Parent = robuxBox

local applyButton = Instance.new("TextButton")
applyButton.Name = "ApplyButton"
applyButton.Size = UDim2.new(1, -18, 0, 36)
applyButton.Position = UDim2.fromOffset(9, 104)
applyButton.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
applyButton.BorderSizePixel = 0
applyButton.Text = "APPLY CHANGES"
applyButton.TextColor3 = Color3.fromRGB(255, 200, 60)
...
applyStroke.Color = Color3.fromRGB(255, 200, 60)
...
robuxBox.FocusLost:Connect(function(enterPressed)
    if enterPressed then applyLocalRobuxAmount() end
end)
...
_G.CartiHubShowBloxFruitsPrompt = showBloxFruitsPromptReplica
_G.CartiHubReplaceRealBloxFruitsOverlay = function()
    for _, root in ipairs(getKnownPurchaseRoots()) do
        if captureAndReplaceRealFoundationPrompt(root, false) then return true end
    end
    return captureAndReplaceRealFoundationPrompt(nil, true)
end
_G.CartiHubSetBloxFruitsWatcher = function(enabled)
    realPromptWatcherEnabled = enabled == true
    foundationWatcherState.Enabled = realPromptWatcherEnabled
end
_G.CartiHubSetBloxFruitsDebug = function(enabled)
    foundationOverlayDebug = enabled == true
end
_G.CartiHubClearBloxFruitsPurchaseOverlays = clearSuppressedRealPurchaseOverlays
_G.CartiHubResetBloxFruitsGiftState = resetGiftState
_G.CartiHubSetBloxFruitsRobux = function(amount)
    localRobuxAmount = math.max(0, tonumber(amount) or localRobuxAmount)
end
_G.CartiHubGetBloxFruitsRobux = function()
    return localRobuxAmount
end
_G.CartiHubSetBloxFruitsProduct = function(name, cost, image)
    if name and tostring(name) ~= "" then productName = tostring(name) end
    if tonumber(cost) then itemRobuxCost = tonumber(cost) end
    if image and tostring(image) ~= "" then productImage = tostring(image) end
end

print("[BloxFruits Standalone] Loaded — gift flow ready, auto-resets after each gift.")